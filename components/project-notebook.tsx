...
function ProjectPreview({ project }: { project: ProjectPortfolioContent }) {
  const stack = previewStack[project.slug] ?? [];

  return (
    <figure className="rounded-[8px] border border-dashed border-border bg-background/45 p-4" aria-labelledby={`${project.slug}-preview-title`}>
      <div className="flex items-center gap-2">
        <BookOpenText className="size-4 text-blue" aria-hidden="true" />
        <figcaption id={`${project.slug}-preview-title`} className="font-mono text-xs uppercase tracking-[0.2em] text-muted-foreground">
          Project preview
        </figcaption>
      </div>
      <div className="mt-4 rounded-md border border-border bg-paper-grid bg-[length:18px_18px] p-4">
        <div className="rounded-md border border-border bg-paper/90 p-4">
          <p className="font-mono text-xs uppercase tracking-[0.16em] text-blue">{project.name}</p>
          <p className="mt-3 text-sm leading-6 text-muted-foreground">{project.architectureSummary}</p>
          <ul className="mt-4 flex flex-wrap gap-2" aria-label={`${project.name} key stack labels`}>
            {stack.map((item) => (
          ...
          <p className="mt-4 rounded-md border border-dashed border-border bg-background/50 px-3 py-2 text-xs text-muted-foreground">
            Project media pending approval.
          </p>
        </div>
      </div>
    </figure>
